...
const TOAST_STATUS = { SUCCESS: 1, DANGER: 2, WARNING: 3, INFO: 4 },
    TOAST_PLACEMENT = {
        TOP_LEFT: 1,
        TOP_CENTER: 2,
        TOP_RIGHT: 3,
...
class Toast {
    static configure(
        maxToasts = null,
        placement = TOAST_PLACEMENT.TOP_RIGHT,
        theme = null,
        enableTimers = !0
    ) {
    ...
    static setPlacement(placement) {
        switch (((TOAST_CONTAINER.className = "position-fixed"), placement)) {
    ...
    static setTheme(theme = null) {
        ...
        switch (theme) {
            case TOAST_THEME.LIGHT:
                (TOAST_TEMPLATE.style.backgroundColor =
                    "var(--body-bg-color-light)"),
    ...
    static _render(toast, timeout) {
        timeout > 0 &&
            ...
            toast.setAttribute("data-autohide", !0));
        ...
        if (enableTimers) {
            ...
            let toastHeader;
            toast.querySelector(".toast-header").removeChild(timer);
        }
        TOAST_CONTAINER.appendChild(toast),
            $(toast).toast("show"),
            currentToastCount++,
            $(toast).on("hidden.bs.toast", () => {
                TOAST_CONTAINER.removeChild(toast), currentToastCount--;
            });
    }
}
